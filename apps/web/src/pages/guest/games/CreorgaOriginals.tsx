...
function RestoLandmark() {
  return (
    <div style={landmarkStyle}>
      <div style={{ width: 48, height: 88, background: 'linear-gradient(180deg, #e2e8f0, #64748b)', clipPath: 'polygon(18% 0, 82% 0, 100% 100%, 0 100%)' }} />
      <div style={{ position: 'absolute', top: 14, width: 30, height: 12, borderRadius: 999, background: '#f59e0b', boxShadow: '0 0 18px rgba(245,158,11,0.8)' }} />
    </div>
  )
}

export function BasketGame({ onBack }: GameProps) {
  const [aim, setAim] = useState(20)
  const [dir, setDir] = useState(1)
  const [score, setScore] = useState(0)
  const [streak, setStreak] = useState(0)
  const [shots, setShots] = useState(10)
  const [last, setLast] = useState('Visez la zone verte puis shootez.')

  useEffect(() => {
    const id = window.setInterval(() => {
      setAim((value) => {
        const next = value + dir * 6
        if (next >= 96) {
          setDir(-1)
          return 96
        }
  ...
  const shoot = () => {
    if (shots <= 0) return
    const distance = Math.abs(aim - 52)
    const made = distance < 8
    const points = distance < 3 ? 3 : made ? 2 : 0
    setShots((value) => value - 1)
    setScore((value) => value + points + (made && streak >= 2 ? 1 : 0))
    setStreak((value) => made ? value + 1 : 0)
    setLast(made ? `Panier ${points} pts${streak >= 2 ? ' + bonus serie' : ''}` : 'Tir manque, ajustez le timing.')
  }
  ...
  const reset = () => {
    ...
    setStreak(0)
    setShots(10)
    setLast('Nouvelle serie de 10 tirs.')
  }

  return (
    <Game3DShell
      title="Basket Rooftop 3D"
      subtitle="Timing arcade, 10 tirs, serie bonus"
      onBack={onBack}
      side={gameSide({
        stats: [
          { label: 'Score', value: score, color: '#f59e0b' },
          { label: 'Tirs', value: shots, color: ACCENT2 },
          { label: 'Serie', value: streak, color: '#22c55e' },
          { label: 'Cible', value: `${Math.round(aim)}%`, color: ACCENT },
        ],
        message: last,
        children: (
          <>
            <ActionButton onClick={shoot} disabled={shots <= 0}>Shooter</ActionButton>
            <button onClick={reset} style={ghostButtonStyle}>Nouvelle serie</button>
          </>
        ),
      })}
    >
      <Stage tone="court">
        <RestoLandmark />
        <div style={basketCourtStyle}>
          <div style={hoopStandStyle}>
            <div style={backboardStyle} />
            <div style={rimStyle} />
            <div style={netStyle} />
          </div>
          <div style={{ ...ballStyle, left: `${aim}%`, transform: `translateX(-50%) translateY(${shots % 2 ? -4 : 0}px)` }} />
          <div style={aimBarStyle}>
            <div style={greenZoneStyle} />
            <div style={{ ...aimNeedleStyle, left: `${aim}%` }} />
          </div>
          <div style={{ color: TEXT, fontWeight: 950, fontSize: 20, textShadow: '0 3px 12px rgba(0,0,0,0.55)' }}>{score} pts</div>
        </div>
      </Stage>
    </Game3DShell>
  )
...
const startingBalls = [
  { id: 1, x: 62, y: 48, color: '#facc15' },
  { id: 2, x: 69, y: 43, color: '#2563eb' },
  { id: 3, x: 69, y: 53, color: '#ef4444' },
  { id: 4, x: 76, y: 38, color: '#7c3aed' },
  { id: 5, x: 76, y: 48, color: '#f97316' },
  { id: 6, x: 76, y: 58, color: '#16a34a' },
]

export function BilliardsGame({ onBack }: GameProps) {
  const [potted, setPotted] = useState<number[]>([])
  const [power, setPower] = useState(55)
  const [message, setMessage] = useState('Rentrez les billes dans l ordre lumineux.')
  const target = startingBalls.find((ball) => !potted.includes(ball.id))

  const shootBall = (id: number) => {
    if (!target) return
    if (id === target.id && power >= 35) {
      const next = [...potted, id]
      setPotted(next)
      setMessage(next.length === startingBalls.length ? 'Table nettoyee.' : `Bille ${id} empochee.`)
    } else if (power < 35) {
      setMessage('Pas assez de puissance.')
    } else {
      setMessage('Faute: jouez la bille indiquee.')
    }
  }

  return (
    <Game3DShell
      title="Billard Lounge 3D"
      subtitle="Pool tactile, ordre de billes et dosage"
      onBack={onBack}
      side={gameSide({
        stats: [
          { label: 'Pochees', value: `${potted.length}/6`, color: '#22c55e' },
          { label: 'Cible', value: target?.id ?? '-', color: '#f59e0b' },
        ],
        message,
        children: (
          <>
            <label style={{ color: MUTED, fontSize: 12, display: 'grid', gap: 6 }}>
              Puissance {power}
              <input type="range" min={10} max={100} value={power} onChange={(event) => setPower(Number(event.target.value))} />
            </label>
            <button onClick={() => { setPotted([]); setMessage('Nouvelle casse.') }} style={ghostButtonStyle}>Nouvelle table</button>
          </>
        ),
      })}
    >
      <Stage tone="pool">
        <div style={poolTableStyle}>
          {[0, 1, 2, 3, 4, 5].map((pocket) => <span key={pocket} style={pocketStyle(pocket)} />)}
          <div style={cueBallStyle} />
          <div style={{ ...cueStyle, width: `${120 + power}px` }} />
          {startingBalls.map((ball) => !potted.includes(ball.id) && (
            <button
              key={ball.id}
              onClick={() => shootBall(ball.id)}
              style={{
                ...poolBallStyle,
                left: `${ball.x}%`,
                top: `${ball.y}%`,
                background: `radial-gradient(circle at 35% 30%, #fff 0 8%, ${ball.color} 9% 100%)`,
                boxShadow: ball.id === target?.id ? '0 0 0 4px rgba(250,204,21,0.25), 0 16px 18px rgba(0,0,0,0.35)' : '0 16px 18px rgba(0,0,0,0.35)',
              }}
            >
              {ball.id}
            </button>
...
const basketCourtStyle: CSSProperties = {
  width: 'min(620px, 100%)',
  minHeight: 430,
  borderRadius: 18,
  border: '1px solid rgba(255,255,255,0.14)',
  background: 'linear-gradient(160deg, rgba(15,23,42,0.42), rgba(2,6,23,0.72)), repeating-linear-gradient(90deg, rgba(251,146,60,0.22) 0 18px, rgba(180,83,9,0.22) 18px 36px)',
  boxShadow: '0 28px 50px rgba(0,0,0,0.4)',
  display: 'flex',
...
const hoopStandStyle: CSSProperties = {
  position: 'absolute',
  top: 28,
  left: '50%',
...
const backboardStyle: CSSProperties = {
  position: 'absolute',
  top: 0,
  left: 38,
  width: 112,
  height: 76,
  borderRadius: 8,
  background: 'linear-gradient(145deg, rgba(248,250,252,0.9), rgba(186,230,253,0.72))',
...
const netStyle: CSSProperties = {
  ...
  borderRight: '2px solid rgba(255,255,255,0.42)',
...
const ballStyle: CSSProperties = {
  ...
  height: 42,
  borderRadius: 999,
  background: 'radial-gradient(circle at 32% 28%, #fed7aa 0 8%, #f97316 9% 100%)',
  border: '2px solid rgba(124,45,18,0.35)',
  boxShadow: '0 18px 24px rgba(0,0,0,0.35)',
  transition: 'left 110ms linear, transform 140ms ease',
}

const aimBarStyle: CSSProperties = {
  width: '82%',
  height: 18,
  borderRadius: 999,
  position: 'relative',
...
const aimNeedleStyle: CSSProperties = {
  position: 'absolute',
  top: -3,
  width: 4,
  height: 24,
...
const poolTableStyle: CSSProperties = {
  width: 'min(760px, 100%)',
  aspectRatio: '1.9',
  position: 'relative',
  borderRadius: 28,
  background: 'radial-gradient(circle at center, #0f766e, #064e3b)',
  border: '18px solid #78350f',
  boxShadow: '0 30px 55px rgba(0,0,0,0.5), inset 0 0 36px rgba(0,0,0,0.34)',
}

function pocketStyle(index: number): CSSProperties {
  const positions = [
    { left: 10, top: 10 },
    { left: 50, top: 4 },
    { right: 10, top: 10 },
...
const cueBallStyle: CSSProperties = {
  position: 'absolute',
  left: '28%',
  top: '50%',
  width: 30,
  ...
  borderRadius: 999,
  transform: 'translate(-50%, -50%)',
  background: 'radial-gradient(circle at 34% 28%, #fff, #cbd5e1)',
  boxShadow: '0 14px 16px rgba(0,0,0,0.3)',
}
...
const cueStyle: CSSProperties = {
  ...
  height: 6,
  borderRadius: 999,
  background: 'linear-gradient(90deg, #fde68a, #92400e)',
  transform: 'translateY(-50%) rotate(-7deg)',
  transformOrigin: 'right',
}

const poolBallStyle: CSSProperties = {
  position: 'absolute',
  width: 32,
  height: 32,
  borderRadius: 999,
