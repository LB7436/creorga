...
- [x] **Validation runtime APPROFONDIE des jeux à risque** (2026-07-12) : **Blackjack** (revue money-logic + main réelle jouée — mise déduite au deal, égalité rembourse la mise → net 0 ✓ ; double-down passe `bet+extra` explicitement à `settle`) et **Reversi** (revue du fix soft-lock `cpuNudge` + **partie complète jouée 4→64/64**, alternance des tours sans gel, victoire 34–30). Aucun bug trouvé.
- [ ] Reste (enhancements uniquement, aucun bug) : polish (états vides, accents FR épars), Web Worker IA échecs, complétude Rami/Rummikub (jokers, manipulation de table)
